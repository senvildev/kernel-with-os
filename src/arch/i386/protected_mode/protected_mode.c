...
{
	vga_text_write_string("info: attempting to enter protected mode");
	__asm__ volatile("cli"); // disable interrupts
	vga_text_write_string("setting up GDT segments");
	...
	vga_text_write_string("succes: set up the GDT segments");

	// load the GDT
	gdt_load();

	vga_text_write_string("info: enabling protected mode");
